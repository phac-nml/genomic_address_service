...
In this example, the first threshold (`5`) defines the maximum distance for the first part of the cluster address ("Component 1"). Cluster assignments for this part of the address will be such that the maximum distance of samples within each cluster from each other is no more than `5` (when performing complete-linkage clustering). So when Sample `A` is assigned cluster label `1` for the first threshold component, we know that Sample `A` is within `5` (complete-linkage) distance from all other samples in cluster `1`. However, note that the distance between samples and clusters depends on the type of linkage clustering method used: *single*, *average*, or *complete*. These linkage methods are explained in more detail later. GAS mcluster uses average-linkage clustering by default, which is the UPGMA algorithm.
