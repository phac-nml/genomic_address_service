...
Once the hierarchical clustering (i.e. linkage) is generated, we can generate flat clusters by choosing a distance threshold, traversing the hierarchical cluster until we find a cluster with a distance that meets our threshold, assigning everything within it the same label, and repeating this process until everything in the hierarchical cluster has been flat clustered with a label.
