...
There are a number of implied clusters that are not shown in the linkage, but are referenced by the linkage. These are clusters with only one element: the original inputs. The linkage can be expanded to improve clarity:
...
For example, if our thresholds are `5,3,0`, we start with the first threshold (`5`) and traverse the linkage (above) from the root until we find a node where the distance is less than our threshold. The root is the last element in the linkage (index `6`).
...
- Index `5` contains `A,B,C` and has a distance of `4`, which is less than the threshold (`5`), so `A,B,C` will become a cluster and we can stop traversing this path.
...
The above example demonstrated creating a hierarchical clustering (linkage) using complete-linkage clustering. *Complete-linkage* clustering distances for merged hierarchical clusters are calculated as the *maximum* distance between each of the two composite clusters in a merged cluster and another cluster:
...
In contrast, *single-linkage* clustering distances for merged hierarchical clusters are calculated as the *minimum* distance between each of the two composite clusters in a merged cluster and another cluster:
...
This process exactly matches the process described in the section for generating an address from a complete-linkage hierarchical clustering. The only difference is that the linkage topology and distances may be different. The following single-linkage is generated using `gas mcluster`:
...
Which corresponds to this meaning:
...
Finally, average-linkage clustering is very similar to single-linkage and complete-linkage clustering, except when calculating updated distances, instead of using the minimum (single) or maximum (complete), *average-linkage clustering* uses the *average* distance.
...
The process for generating flat cluster addresses from an average-linkage hierarchical clustering is the same as for single- and complete-linkage clustering. However, the topology and distances in the linkage may be different from the linkage methods, which may result in slightly different flat clusters and addresses.
...
We've previously generated a square distance matrix using profile_dists. We then generated a hierarchical clustering and linkage from those distances and converted the data structures into flat clusters using gas mcluster. We'd like to add additional samples to our flat clusters (i.e. addresses) without having to repeat the entire process from scratch with the additional samples added into the input. gas call allows us to update an existing cluster. However, we need pairwise distances between new samples and existing samples in the cluster so that we know how where to place the new samples.
